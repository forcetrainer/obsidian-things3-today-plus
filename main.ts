...
export class ThingsView extends ItemView {
	...
	async getAndShowTodayTodos() {
		...
		// Upcoming button
		const upcomingBar = container.createEl("div", {cls: "things3-upcoming-toggle-bar"});
		const upcomingBtn = upcomingBar.createEl("button", {text: "Show Upcoming", cls: "things3-btn"});
		upcomingBtn.addEventListener("click", () => {
			window.open("things:///show?id=upcoming");
		});
